...
Me: Less parameters can be removed by PCA, and makes it less overfit. 
Bias vs Variance
(Loss function + sum of square of parameters) x Weight drop [which should be 0.1]
Learner constructs with wd, all of them do. So try wd=0.1 instead of default wd=0.01.
...
w[T] = w[T-1] - r(dL/dw[T-1])
L(x,w) = MSE(predicted[M(x,w)]vs,actual)
...
ME: Momentum, when your next step is S[t]= a * gradient + (1-a)S[t-1], where a is usually 0.1.
